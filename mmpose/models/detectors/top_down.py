...
class TopDown(BasePose):
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        if 'teacher_output_heatmap' in data_batch:
            losses = self.forward_distill(data_batch['img'], data_batch['target'],
                            data_batch['target_weight'], data_batch['teacher_output_heatmap'], **kwargs)
        else:
    ...
    def forward_distill(self, img, target, target_weight, teacher_target, **kwargs):
        ...
        if self.with_keypoint:
            ...
            t_keypoint_losses = self.keypoint_head.get_loss(
                output, teacher_target, target_weight)
            loss_dict = dict(list(keypoint_losses.items()) + list({'kd_heatmap_loss': t_keypoint_losses['heatmap_loss']}.items()))
    ...
    #design frame difference delta_feature
    def disdill_delta_feature_get(self, img, target, target_weight, teacher_target, **kwargs):
        """Defines the computation performed at every call when computing delta."""
        img_feature=list()
        output=self.backbone(img)
        delta_feature=output - img_feature[len(img_feature)]
